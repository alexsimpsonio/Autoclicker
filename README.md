...
2) You can change:
   ...
   exp:  
   max_run_time is a counter, once reached the AutoClicker stops. EG if you want to run it for two hours change max_run_time to 120 * 60.
   ...
   exp:
   These determine the random break length discussed above, the min and max are the thresholds for how long a break is taken during the run time.
   ...
   exp:
   offsets determine how much the x and y co-ordinate is changed each click intended to mimic human inconsistencies. Adjust to your needs, sometimes they might be too large.
